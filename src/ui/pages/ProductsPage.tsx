...
import type { CatalogCollectionInfo } from '@infrastructure/server/loadCatalogBootstrap';
...
export function ProductsPage({
  ...
  const hasBootstrap = initialProducts !== undefined;
  ...
  const [collectionInfo, setCollectionInfo] = useState<CatalogCollectionInfo | null>(
  ...
  useEffect(() => {
    ...
    void loadMetadata();
    return () => controller.abort();
  }, [collectionSlug, resolvedType, services, initialCollectionInfo]);
              ...
              {nextCursor && !debouncedSearch && (
                <div className="mt-20 text-center">
                  <button
                    ...
                    className="inline-flex items-center gap-3 rounded-4xl bg-gray-900 px-12 py-5 font-black text-xs uppercase tracking-widest text-white shadow-2xl transition hover:bg-black hover:-translate-y-1 active:translate-y-0 disabled:opacity-70 disabled:hover:translate-y-0"
                  >
                    {loadingMore ? (
                      <>
