...
import {
  ...
  guestCartPayload,
  type MockCartLine,
} from './helpers/cartProtocol';

/**
 * [TEST SUITE: Storefront release smoke — cart protocol aware mocks]
 */
test.describe('Comprehensive Cart and Checkout Flow', () => {
  ...
  async function setupBaseMocks(page: Page) {
    ...
    await page.route('/api/cart/**', async (route) => {
      ...
      if (url.includes('mixed')) {
        items.push({ productId: 'p2', name: 'Digital Art', priceSnapshot: 2000, quantity: 1 });
      }

      if (url.includes('/merge-guest')) {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify(
      ...
      if (url.includes('/validate') && method === 'POST') {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify(cartOk({ valid: true, issues: [], requiresRefresh: false })),
        });
        return;
      }

      const view = buildCartView(items, url.includes('auth') ? 'u1' : 'guest');
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
  ...
  test('should require shipping for mixed carts', async ({ page }) => {
    await seedCart(page, [
      { productId: 'p1', name: 'Physical Art', priceSnapshot: 5000, quantity: 1 },
      { productId: 'p2', name: 'Digital Art', priceSnapshot: 2000, quantity: 1 },
    ]);
